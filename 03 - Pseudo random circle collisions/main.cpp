...
int main()
{
    ...
    while (window.isOpen()) 
    {
        ...
        for (int i = 0; i < NUMBER_OF_CIRCLES; i++)
        {
            ...
            if (circles[i].getPosition().x > 800 || circles[i].getPosition().x < 0 || circles[i].getPosition().y > 600 || circles[i].getPosition().y < 0)
            {
                movement[i] = -movement[i];

                circles[i].setRotation(circles[i].getRotation() + 10);
            }
        }
